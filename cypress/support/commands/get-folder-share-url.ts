...
export default function getFolderShareUrl(
  ...
  const folderIdAlias: string | null = folderIdOrAlias.startsWith('@') ? folderIdOrAlias : null
  const folderId: string = !!folderIdAlias ? this[folderIdAlias.slice(1)] : folderIdOrAlias
  ...
  const url = new URL(`/share/${folderIdOrAlias}:${folderName}`, Cypress.config().baseUrl)
...
declare global {
  namespace Cypress {
    interface Chainable {
      getFolderShareUrl(folderIdOrAlias: string, folderName: string): Chainable<string>
