...
describe("Managing files", () => {
  ...
  it("should be possible to cancel an upload", () => {
    ...
    cy.get("@uploadProgress")
      .contains("File upload aborted by you")
      .should("be.visible");
  ...
  it("should be possible to consult the public shareable link anonymously, download all files and keep download counts", () => {
    ...
    cy.contains(
      ...
      .then(function () {
        ...
        cy.readFile("cypress/downloads/test.txt").should("not.exist");
        cy.contains("table tbody tr", "test.tx")
          .find("td")
          .last()
          .contains("a", "Download")
          .click();
        ...
        cy.readFile("cypress/downloads/test.pdf").should("not.exist");
        cy.contains("table tbody tr", "test.pdf")
          .find("td")
          .last()
          .contains("a", "Download")
          .click();
        ...
        cy.readFile("cypress/downloads/test.json").should("not.exist");
        cy.contains("table tbody tr", "test.json")
          .find("td")
          .last()
          .contains("a", "Download")
          .click();
  ...
  function assertNumberOfDownloads(
    ...
    cy.contains("table tbody tr", fileName)
      .find("td")
      .eq(3)
      .should("have.text", expectedNumberOfDownloads);
  }
  ...
  function assertFileDelete(
    ...
    cy.contains("#files table tr", fileToDelete)
      .contains(".btn", "Delete")
      .click();
  ...
  function assertFolderFileCount(FILE_COUNT: number) {
    ...
    cy.contains("table tr", FOLDER_NAME)
      .find("td")
      .eq(3)
      .should("contain", `${FILE_COUNT} files`);
